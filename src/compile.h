...
block jq_gen_location(location, struct locfile*, block);
...
block jq_gen_call(const char* name, block body);
...
block jq_gen_destructure(block var, block matcher, block body);
...
block jq_gen_cbinding(const struct cfunction* functions, int nfunctions, block b);
