...
static block bind_bytecoded_builtins(block b) {
  block builtins = jq_gen_noop();
  {
    struct bytecoded_builtin builtin_defs[] = {
      {"empty", jq_gen_op_simple(BACKTRACK)},
      {"not", jq_gen_condbranch(jq_gen_const(jv_false()),
                             jq_gen_const(jv_true()))}
    };
    for (unsigned i=0; i<sizeof(builtin_defs)/sizeof(builtin_defs[0]); i++) {
      builtins = BLOCK(builtins, jq_gen_function(builtin_defs[i].name, jq_gen_noop(),
                                              builtin_defs[i].code));
    }
  }
  {
    struct bytecoded_builtin builtin_def_1arg[] = {
      {"path", BLOCK(jq_gen_op_simple(PATH_BEGIN),
                     jq_gen_call("arg", jq_gen_noop()),
                     jq_gen_op_simple(PATH_END))},
    };
    for (unsigned i=0; i<sizeof(builtin_def_1arg)/sizeof(builtin_def_1arg[0]); i++) {
      builtins = BLOCK(builtins, jq_gen_function(builtin_def_1arg[i].name,
                                              jq_gen_param("arg"),
  ...
  }
  {
    ...
    block rangevar = jq_gen_op_var_fresh(STOREV, "rangevar");
    block rangestart = jq_gen_op_var_fresh(STOREV, "rangestart");
    block range = BLOCK(jq_gen_op_simple(DUP),
                        jq_gen_call("start", jq_gen_noop()),
                        rangestart,
                        jq_gen_call("end", jq_gen_noop()),
                        jq_gen_op_simple(DUP),
                        jq_gen_op_bound(LOADV, rangestart),
                        // Reset rangevar for every value generated by "end"
                        rangevar,
                        jq_gen_op_bound(RANGE, rangevar));
    builtins = BLOCK(builtins, jq_gen_function("range",
                                            BLOCK(jq_gen_param("start"), jq_gen_param("end")),
                                            range));
  }
...
static block jq_gen_builtin_list(block builtins) {
  jv list = jv_array_append(block_list_funcs(builtins, 1), jv_string("builtins/0"));
  return BLOCK(builtins, jq_gen_function("builtins", jq_gen_noop(), jq_gen_const(list)));
}
...
int builtins_bind(jq_state *jq, block* bb) {
  ...
  builtins = jq_gen_cbinding(function_list, sizeof(function_list)/sizeof(function_list[0]), builtins);
  builtins = jq_gen_builtin_list(builtins);
