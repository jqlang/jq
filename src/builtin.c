...
static jv f_plus(jq_state *jq, jv input, jv a, jv b) {
  ...
  if (jv_get_kind(a) == JV_KIND_NULL) {
    ...
    if (jv_get_string_kind(a) != JV_STRING_KIND_UTF8 ||
        jv_get_string_kind(b) != JV_STRING_KIND_UTF8)
      return jv_binary_concat(a, b);
...
static jv f_concat(jq_state *jq, jv a, jv b) {
  if (jv_get_kind(a) == JV_KIND_NULL) {
    jv_free(a);
    return b;
  } else if (jv_get_kind(b) == JV_KIND_NULL) {
    ...
    if (jv_get_string_kind(a) == JV_STRING_KIND_UTF8) {
      if (c < 0 || c > 0x10FFFF)
        return type_error2(a, b, "cannot add invalid Unicode codepoint to string");
      jv_free(b);
      return jv_string_append_codepoint(a, c);
    }
    if (c < 0 || c > 255)
        return type_error2(a, b, "cannot add invalid byte value to string");
    jv_free(b);
    return jv_binary_append_byte(a, c);
  } else if (jv_get_kind(a) == JV_KIND_STRING && jv_get_kind(b) == JV_KIND_ARRAY) {
    if (jv_get_string_kind(a) == JV_STRING_KIND_UTF8) {
      jv_array_foreach(b, bi, belem) {
        if (jv_get_kind(belem) == JV_KIND_ARRAY) {
          a = f_concat(jq, a, belem);
          if (!jv_is_valid(a)) {
            jv_free(b);
            return a;
          }
        ...
        if (jv_get_kind(belem) == JV_KIND_STRING) {
          a = f_concat(jq, a, belem);
          if (!jv_is_valid(a)) {
            jv_free(b);
            return a;
        ...
        if (jv_get_kind(belem) != JV_KIND_NUMBER) {
          jv_free(b);
          return type_error2(a, belem, "cannot convert array of non-Unicode codepoint values to concatenate to string");
        }
        int c = jv_number_value(belem);
        if (c < 0 || c > 0x10FFFF) {
          jv_free(b);
          return type_error2(a, belem, "cannot convert array of non-Unicode codepoint values to concatenate to string");
        }
        jv_free(belem);
        a = jv_binary_append_byte(a, (unsigned char)c);
      }
    } else {
      jv_array_foreach(b, bi, belem) {
        if (jv_get_kind(belem) == JV_KIND_ARRAY) {
        ...
        if (jv_get_kind(belem) != JV_KIND_NUMBER) {
          jv_free(b);
          return type_error2(a, belem, "cannot convert array of non-byte values to concatenate to string");
        }
        int c = jv_number_value(belem);
        if (c < 0 || c > 255) {
          jv_free(b);
...
static jv f_format(jq_state *jq, jv input, jv fmt) {
  ...
  if (!strcmp(fmt_s, "json")) {
    ...
    jv_free(fmt);
    input = jv_binary_from_base64(f_tostring(jq, input));
    if (!jv_is_valid(input))
      return input;
    input.subkind = JV_STRING_KIND_BINARY_UTF8;
    return jv_string_from_binary(input);
  } else if (!strcmp(fmt_s, "base64dbinary")) {
    jv_free(fmt);
    return jv_binary_from_base64(f_tostring(jq, input));
  } else if (!strcmp(fmt_s, "hex")) {
    jv_free(fmt);
    return jv_binary_to_hex(f_tostring(jq, input));
  } else if (!strcmp(fmt_s, "hexd")) {
    jv_free(fmt);
    return jv_binary_from_hex(f_tostring(jq, input));
  } else {
    jv_free(input);
...
static jv f_isbinary(jq_state *jq, jv a) {
  if (jv_get_kind(a) != JV_KIND_STRING || jv_get_string_kind(a) == JV_STRING_KIND_UTF8) {
    jv_free(a);
    return jv_false();
  }
  jv_free(a);
...
static jv f_encodeas(jq_state *jq, jv a, jv b) {
  ...
  if (strcmp(as, "UTF-8") == 0 || strcmp(as, "utf-8") == 0) {
    ...
    a.subkind = JV_STRING_KIND_BINARY;
  } else if (strcmp(as, "hex") == 0) {
    a.subkind = JV_STRING_KIND_BINARY_HEX;
...
static jv f_encoding(jq_state *jq, jv a) {
  ...
  case JV_STRING_KIND_BINARY_HEX: return jv_string("hex");
...
#include "libm.h"
...
#endif
  {(cfunction_ptr)f_concat, "concat", 2},
  ...
  {(cfunction_ptr)f_isbinary, "isbinary", 1},
